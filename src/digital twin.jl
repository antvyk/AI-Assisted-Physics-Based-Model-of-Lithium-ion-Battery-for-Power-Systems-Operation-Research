...
function Final_SoC_Caploss(SoC_init,PowerInput,SoC_max_init)
    ...
    P = repeat(PowerInput, inner=trunc(Int,(3600/dt)))
    ...
    I = P[1]/(OCP_p_spline(theta_p) - OCP_n_spline(theta_n))
    ...
    SoC_final = SoC
    Capacity_final = SoC_max_init-LLI
    
    return SoC_final, Capacity_final
end
######################################################################################
...
file_name_from_optimization = 
Dispatch_Year = DataFrame(XLSX.readtable(file_name_from_optimization, "RESULTS"))
...
for day in 1:365
    ...
    SoC_init = 0.1
    global SoC_max_init
    PowerInput = Dispatch_Year[1:end,:"x"*string(1+day)]
    SoC_final, SoH_final = Final_SoC_Caploss(SoC_init,PowerInput,SoC_max_init)
    SoC_max_init = SoH_final
    Output[day,1] = day
    Output[day,2] = SoC_final
    Output[day,3] = SoH_final
    
     println("________________________________________")
    println("DAY: ---> ",day)
    println("________________________________________")
    println()
    println("Final SoC: ---> ", SoC_final)
    println()
    println()
    println("Final SoH: ---> ", SoH_final)
    println()
end


Results_in_df_Sim = DataFrame(Output,[:Day,:SoC_Day_End,:SoH_Day_End])


Results_output_file_name = "YEAR_1_Simulation_from_NN_LLI_ONLY_year2022"
if isfile(directory_out*Results_output_file_name*".xlsx") == true
   rm(directory_out*Results_output_file_name*".xlsx", force=true)
end
XLSX.writetable(directory_out*Results_output_file_name*".xlsx", RESULTS=(collect(DataFrames.eachcol(Results_in_df_Sim)), DataFrames.names(Results_in_df_Sim) ))
